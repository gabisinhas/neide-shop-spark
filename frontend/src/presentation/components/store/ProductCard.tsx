...
import { getDisplayOriginalPrice, getEffectiveProductPrice } from "@/shared/utils/productPricing";
...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  const originalDisplayPrice = getDisplayOriginalPrice(product);
      ...
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        {originalDisplayPrice && originalDisplayPrice > effectivePrice && (
          <span className="absolute top-3 right-3 bg-destructive text-destructive-foreground px-2 py-1 rounded-full text-xs font-bold">
            {Math.round((1 - effectivePrice / originalDisplayPrice) * 100)}% OFF
          </span>
        )}
      </div>
      <div className="p-4">
        ...
        <div className="mb-3">
          {originalDisplayPrice && originalDisplayPrice > effectivePrice && (
            <p className="text-xs text-muted-foreground line-through">
              R$ {originalDisplayPrice.toFixed(2).replace(".", ",")}
            </p>
          )}
