...
const CartDrawer = () => {
  ...
  const handleCheckout = () => {
    setIsOpen(false);
    navigate(isAuthenticated ? APP_ROUTES.checkout : APP_ROUTES.auth, {
      state: { from: APP_ROUTES.checkout },
    });
  };
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display font-bold text-lg text-foreground">Carrinho ({totalItems})</h2>
              </div>
              <button
                onClick={() => setIsOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {items.length === 0 && <p className="text-center text-muted-foreground py-12">Seu carrinho está vazio</p>}
                ...
                <div key={product.id} className="flex gap-3 bg-background rounded-lg p-3">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-sm font-bold text-primary">R$ {getEffectiveProductPrice(product).toFixed(2).replace(".", ",")}</p>
                    </div>
            ...
            {items.length > 0 && (
              <div className="p-4 border-t border-border space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-gradient-gold text-primary-foreground py-3 rounded-full font-bold text-sm hover:shadow-gold transition-shadow"
                >
                  {isAuthenticated ? 'Ir para o checkout' : 'Entrar para finalizar'}
                </button>
                <p className="text-xs text-center text-muted-foreground">
                  Seus pedidos ficam salvos para acompanhamento no painel da conta.
                </p>
              </div>
            )}
          </motion.div>
